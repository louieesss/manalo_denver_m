...
<body class="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 via-white to-emerald-100 font-sans text-gray-800 p-6">
  ...
  <div class="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 space-y-8">
    <h1 class="text-3xl font-semibold text-emerald-700 mb-6 text-center">Update Record</h1>
    ...
    <form action="<?=site_url('users/update/'.$user['id']);?>" method="post" class="space-y-6">
      ...
      <div>
        ...
        <input type="text" id="last_name" name="last_name" value="<?=html_escape($user['last_name']);?>"
          class="w-full mt-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-400 focus:outline-none transition"/>
      </div>
      ...
      <button type="submit" 
        class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-3 rounded-lg shadow-md transition duration-200">
        Update
      </button>
